...
void PacManSystem::update() {
	...
	if (ihldr.keyDownEvent()) {
		...
		if (ihldr.isKeyDown(SDL_SCANCODE_RIGHT) || ihldr.isKeyDown(SDL_SCANCODE_D)) { // rotate right
		...
		else if (ihldr.isKeyDown(SDL_SCANCODE_LEFT) || ihldr.isKeyDown(SDL_SCANCODE_A)) { // rotate left
			_pmTR->_rot = _pmTR->_rot - 90.0f;
		...
		else if (ihldr.isKeyDown(SDL_SCANCODE_UP) || ihldr.isKeyDown(SDL_SCANCODE_W)) { // increase speed
			_pmTR->_vel = Vector2D(0.0f, -3.0f).rotate(_pmTR->_rot);
		}
		else if (ihldr.isKeyDown(SDL_SCANCODE_DOWN) || ihldr.isKeyDown(SDL_SCANCODE_S)) { // decrease speed
